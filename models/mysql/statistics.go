...
var icount = &monitor.Metric{
    ID:          "mysql_request_count",
    Name:        "mysql_request_count",
    Description: "MySQL requests detail, partitioned by function and error.",
    Type:        "counter_vec",
    Args:        []string{ "func", "errmsg" },
}

var iduration = &monitor.Metric{
    ID:          "mysql_request_duration",
    Name:        "mysql_request_duration",
    Description: "The MySQL request latencies in milliseconds.",
    Type:        "histogram_vec",
    Args:        []string{ "func" },
}
